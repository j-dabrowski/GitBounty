...
async function main() {
  ...
  async function deploy_escrow(owner, main) {
    ...
    const result = await main.createEscrow(
      owner.address,
      "testName",
      "19",
      "notional"
    ...
    const OtherIsEscrow = await main.isEscrow(
      "0xCC1ddA58ec286dbb5deA3D2B377A9cFcCF924a5a"
    );
  ...
  async function deploy_consumer(mainAddress) {
    ...
    const oracleAddress = ethers.utils.getAddress(
      "0xeA6721aC65BCeD841B8ec3fc5fEdeA6141a0aDE4"
    ); //For Mumbai.No change if used Mumbai
  ...
  async function createSubscription(consumerAddress) {
    ...
    console.log(
      `Funding with ` + juelsAmount + ` Juels (1 LINK = 10^18 Juels)`
    );
